...
- `git`
- One supported agent binary (`claude`, `codex`, `cursor-agent`, `acli` for Rovo Dev, `opencode`, `pi`, or `copilot`), or a separately installed `acpx` binary for `agent: acp:<target>`
