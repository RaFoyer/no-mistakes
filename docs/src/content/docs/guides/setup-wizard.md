...
committing local changes, and pushing through the gate, then attach if shared-
runtime admission succeeds and the daemon registers the new run. This is the
setup wizard.
...
wizard helps turn that state into branch -> commit -> push -> attach when
admission succeeds and the daemon registers the new run.
...
flowchart TD
  ...
  push --> admitted{"Admission granted?"}
  admitted -- "no" --> error["Show admission error"]
  admitted -- "yes" --> registered{"Run registered?"}
...
If the push succeeds, the interactive wizard stays visible in a brief `waiting for run…` state while the daemon obtains admission and registers the new run, then hands off to the main TUI and attaches. If admission is unavailable or no run appears in time, the wizard exits with an error instead of silently falling through, and points you at the admission or gate `notify-push.log` diagnostics.
