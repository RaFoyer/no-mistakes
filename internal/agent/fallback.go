...
func (a *fallbackAgent) Run(ctx context.Context, opts RunOpts) (*Result, error) {
	...
	for i, current := range candidates {
		if opts.Routing.EffectiveModel != "" && !strings.EqualFold(current.Name(), "codex") && !strings.EqualFold(current.Name(), "cursor") {
			return nil, fmt.Errorf("agent %q cannot enforce the configured model routing policy; refusing fallback", current.Name())
		}
		currentOpts := opts
