...
func TestNewPipelineAgent_OptOut_RefusesUnverifiedHarness(t *testing.T) {
	for _, name := range []types.AgentName{types.AgentCursor, types.AgentOpenCode, types.AgentPi, types.AgentCopilot} {
		cfg := &config.Config{Agent: name, DisableProjectSettings: true}
...
func TestNewPipelineAgent_NoOptOut_AdmitsEveryHarness(t *testing.T) {
	...
	for _, name := range []types.AgentName{types.AgentCodex, types.AgentClaude, types.AgentCursor, types.AgentOpenCode, types.AgentPi, types.AgentCopilot} {
