...
- Effective model/effort fields describe only controls the concrete adapter actually received. Attempts other than Codex and Cursor keep them empty, fallback rows are attributed to each attempted adapter, and a Codex- or Cursor-effective route fails closed before an unenforceable fallback launches. Inspect both evidence streams through the read-only `no-mistakes axi route-evidence` surface.
